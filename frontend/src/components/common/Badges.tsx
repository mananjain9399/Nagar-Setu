...
export const ChannelBadge: React.FC<{ channel: string }> = ({ channel }) => {
  ...
  if (c === 'CM_HELPLINE_181') {
    return (
      <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-mono font-semibold bg-purple-50 text-purple-700 border border-purple-200">
        CM 181
      </span>
    );
  }
  ...
  if (c === 'SOCIAL_MEDIA') {
    return (
      <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-mono font-semibold bg-blue-50 text-blue-700 border border-blue-200">
        Social Media
      </span>
    );
  }
