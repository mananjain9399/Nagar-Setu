...
export const QueueFilterToolbar: React.FC<QueueFilterToolbarProps> = ({
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-md p-3.5 shadow-xs space-y-3">
      ...
      <div className="flex flex-col sm:flex-row gap-2.5 items-center justify-between">
        <div className="relative w-full sm:w-80">
          ...
          <input
            type="text"
            placeholder="Search grievance, external ID, location..."
            value={filters.search || ''}
            onChange={(e) => onFilterChange({ search: e.target.value, page: 1 })}
            className="w-full pl-8 pr-3 py-1.5 bg-slate-50 border border-slate-200 rounded text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-blue-600 focus:bg-white transition-all font-medium"
          />
        </div>

        <div className="flex items-center gap-3 self-end sm:self-auto text-xs">
          <span className="text-slate-600">
            Showing <strong className="text-slate-900 font-mono font-bold">{totalFiltered}</strong> matching records
          </span>
          <button
            onClick={onResetFilters}
            className="flex items-center gap-1.5 px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded text-xs border border-slate-200 font-semibold transition-all cursor-pointer"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-9 gap-2 pt-2 border-t border-slate-100">
        ...
        <div>
          ...
          <select
            ...
            className="w-full bg-slate-50 border border-slate-200 rounded py-1 px-1.5 text-xs text-slate-800 font-medium focus:outline-none focus:border-blue-600"
          >
